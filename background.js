...
if (contentTypeHeader && !isBlacklistedContentType(contentTypeHeader.value))
  ...
  if (!contentType) contentType = guessContentType(request.url);
  if (contentType) {
    if (contentTypeHeader) contentTypeHeader.value = contentType;
    else request.responseHeaders.push({ name: "Content-Type", value: contentType });
  }
  return {
